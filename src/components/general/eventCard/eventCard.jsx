...
const EventCard = ({ socket }) => {
  ...
  const sendJoinRequest = async () => {
    try {
      const data = await doApiMethod(
        API_URL + `/events/addJoinRequest/${event?._id}`,
        "patch"
      );
  ...
  const addPraticipent = async () => {
    try {
      const data = await doApiMethod(
        API_URL + `/events/addParticipant/${event?._id}/${user_id}`,
        "patch"
      );
      setPraticipant(true);
      setShowNotification(true);
  ...
  const addReview = async () => {
    ...
    try {
      let review = await doApiMethod(
        API_URL + `/reviews/addReview/${event?._id}`,
        "post",
        reviewBody
      );
      review = review.data;
  ...
  const removeReviews = async (index) => {
    try {
      ...
      const data = await doApiMethod(
        API_URL + `/reviews/deleteReview/${review?._id}`,
        "delete"
      );
      const updatedReviews = [...reviews];
  ...
  const editReview = async (index, review) => {
    try {
      ...
      setReviews(updatedReviews);
      const data = await doApiMethod(
        API_URL + `/reviews/editReview/${review._id}`,
        "put",
        body
      );
    } catch (err) {
  ...
  const doApiGetEvent = async () => {
    try {
      ...
      if (
        resp.data?.participants?.some(
          (participant) => participant._id === userInfo?.user?._id
        )
      ) {
        setPraticipant(true);
      }
        ...
        <div className="container-fluid ">
          <div className="container pb-4">
            <div className="row  pt-5">
              {/* event information */}
              <div
                className="col-12 col-md-8  p-5 border"
              ...
              >
                ...
                <div className="row">
                  <div className="col-6 col-lg-4">
                    <IonCol>
                      <IonRow>
                        ...
                        <IonCol size="9">{event?.address?.state}</IonCol>
                      </IonRow>
                      ...
                      {!event?.price?.free ? (
                          ...
                          <IonRow>
                            {event?.price?.studentOrSoldier && (
                              <>
                                ...
                                <IonCol size="9">
                                  {" "}
                                  {event?.price?.studentOrSoldier} ש"ח
                                </IonCol>
                              </>
                            )}
                          </IonRow>
                        ...
                        <IonRow className="pt-4">
                          <IonCol size="3">
                            {" "}
                            <i className="fa fa-pagelines fa-2x"></i>
                          </IonCol>
                    ...
                    <IonCol>
                      <IonCol>
                        ...
                        <IonRow>
                          <IonCol size="3">
                            {" "}
                            <i className="fa fa-wheelchair-alt fa-2x"></i>
                          </IonCol>
                          <IonCol size="9">
                            {" "}
                            {event?.accessibility
                  ...
                  >
                    ...
                    <div style={{ fontSize: "24px" }}>
                      {new Date(event?.date_and_time).toLocaleTimeString(
                        "en-GB",
                        {
                          hour: "2-digit",
                          minute: "2-digit",
              ...
              <p
                            ...
                            {openEvent && (
                              <IonCol size="2">
                                ...
                                >
                                  <i className="fa   fa-hand-peace-o fa-2x">
                                    {" "}
                                  </i>
                                  <span style={{ fontSize: "small" }}>
                                    אני רוצה לבוא
                  ...
                  style={{
                    backgroundImage:
                      event.images.length > 0
                        ? `url(${event.images[currentImageIndex]})`
                        : `url(${require("../../../images/bus.jpg")})`,
            ...
            {/* event map */}
            {event && <EventsMap events={[event]} />}

            <div className="row mt-4 ">
              {/* event reviews */}
              <div className=" col-12 col-md-4 p-0">
                <div
                  className="px-2 chat border ms-md-3 mb-4 mb-md-0"
                  style={{
                    maxHeight: "650px",
                ...
                >
                  <p
                    className="text-center p-3 h4 m-4"
                    style={{
                      color: "#077F7A",
                      borderBottom: "1px solid #077F7A",
                    }}
                  >
                    דירוגים:
                  </p>
